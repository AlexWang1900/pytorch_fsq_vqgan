...
class VQGANTransformer(nn.Module):
    def __init__(self, args):
        ...
        self.transformer = GPT(config =transformer_config)
    ...
    def z_to_image(self, indices, p1=8, p2=8):#16,16
        ix_to_vectors = self.vqgan.codebook.indices_to_codes(indices).reshape(indices.shape[0], p1, p2, 4)#
        ix_to_vectors = ix_to_vectors.permute(0, 3, 1, 2)
        image = self.vqgan.decode(ix_to_vectors)
        return image

    def forward(self, x):#([32, 3, 128, 128])
        _, indices = self.encode_to_z(x)#([32, 64])

        sos_tokens = torch.ones(x.shape[0], 1) * self.sos_token#([32, 1]) value = 0
        ...
        target = indices.long()#([20, 64])

        logits, loss = self.transformer(new_indices[:, :-1],target)#([20, 64, 1024])
