...
class TrainTransformer:
    def __init__(self, args):
        ...
        self.optim = self.model.transformer.configure_optimizers(weight_decay = 0.01, learning_rate = 4.5e-06, betas= (0.9, 0.95), device_type='cuda')
    ...
    def train(self, args):
        ...
        for epoch in range(args.epochs):
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):#([20, 3, 128, 128])
                    self.optim.zero_grad()
                    imgs = imgs.to(device=args.device)#([20, 3, 128, 128])
                    logits, loss = self.model(imgs)#([20, 64, 1024]),([20, 64])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--levels',default = [8,5,5,5])
    parser.add_argument('--image-size', type=int, default=128, help='Image height and width.)')
    parser.add_argument('--num-codebook-vectors', type=int, default=1000, help='Number of codebook vectors.')
    parser.add_argument('--beta', type=float, default=0.25, help='Commitment loss scalar.')
    parser.add_argument('--image-channels', type=int, default=3, help='Number of channels of images.')
    parser.add_argument('--dataset-path', type=str, default='./data', help='Path to data.')
    parser.add_argument('--checkpoint-path', type=str, default='./checkpoints/last_ckpt.pt', help='Path to checkpoint.')
    parser.add_argument('--device', type=str, default="cuda", help='Which device the training is on')
    parser.add_argument('--batch-size', type=int, default=32, help='Input batch size for training.')#20
    ...
    args.dataset_path = './data/FFHQ_128'#r"C:\Users\dome\datasets\flowers"
    args.checkpoint_path = './checkpoints/vqgan_epoch_12.pt'#r".\checkpoints\vqgan_last_ckpt.pt"
